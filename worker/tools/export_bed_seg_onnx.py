...
def export_bed_seg_onnx(
    ...
    with tempfile.TemporaryDirectory(dir=source.parent, prefix="bed-seg-export-") as work:
        ...
        try:
            ...
            exported = Path(YOLO(temporary_weights).export(format="onnx", imgsz=imgsz, opset=17))
    ...
    if existing_digest is not None and existing_digest != digest and not force:
        raise ModelLoadError("yolo26l-seg.onnx differs; pass --force to overwrite it")
...
def main() -> None:
    ...
    print(export_bed_seg_onnx(args.model_path, force=args.force, imgsz=args.imgsz))
