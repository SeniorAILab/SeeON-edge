...
from worker.runtime.clip_analysis_execution import execute_job, settle_job
...
class ClipAnalysisSupervisor:
    ...
    def cancel(self, clip_id: str) -> bool:
        with self._condition:
            if self._queue.remove(clip_id) or self._preparing == clip_id:
                self._cancelled.add(clip_id)
    ...
    def shutdown(self) -> None:
        with self._condition:
            ...
            if self._active is not None:
                self._cancelled.add(self._active.clip_id)
            process = self._process
            self._condition.notify_all()
        if process is not None:
            self._terminate_group(process)
    ...
    def _serve(self) -> None:
        while True:
            with self._condition:
                ...
                self._preparing = pending.clip_id
            ...
            with self._condition:
                ...
                if self._stopping or pending.clip_id in self._cancelled:
            ...
            status = execute_job(
                ...
                self._launch,
                self._set_process,
                lambda clip_id=job.clip_id: self._is_cancelled(clip_id),
    ...
    def _settle(
        ...
        with self._condition:
            process = self._process
        status, teardown_failed = settle_job(
            job,
            status,
            process,
            identity=lambda queued: artifact_identity(
                queued, self._pose_model, self._bed_model, self._profile_sha
            ),
    ...
    def _set_process(self, process: subprocess.Popen[bytes]) -> bool:
        with self._condition:
            ...
            return self._stopping or (
                self._active is not None and self._active.clip_id in self._cancelled
            )

    def _is_cancelled(self, clip_id: str) -> bool:
        with self._condition:
            return self._stopping or clip_id in self._cancelled
