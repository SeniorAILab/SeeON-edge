...
- 이슈 #217, #218, #219, #220, #226, #238 — 이 문서의 표/판단 트리가 인용하는
  원본 근거.
- PR #234(#218 수정, 오픈/미머지), #241(#238 수정 — 이 런북의 B절이 다루는
  (b)/(c) 구분 신호, 오픈/미머지, head `5e8fd29`) — 아직 오픈 상태이고 이
  문서의 판단 트리 정확도에 직접 영향을 준다. PR #242(로그 렌더링 갭 수정)는
  **머지됨**(`6f35357`) — A-1/로그 확인 절이 그 상태를 반영한다.
