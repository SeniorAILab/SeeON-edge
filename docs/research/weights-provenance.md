...
- `worker/adapters/model/sklearn_fall.py:86-150` loads the sklearn fall
  registry model's joblib model and metadata; retained as a loadable adapter
  (see `worker/adapters/model/__init__.py`) but no longer wired as an
...
| `models/pose/yolo26n-pose.onnx` | Dynamic-batch engine-build input; produced by `worker/tools/export_pose_onnx.py` from the pose checkpoint | **unknown** | **unknown** | SHA-256 `724ae1b1b4420cea85f98ac7c5ccabf3d56229f7c25390475e09b7056e3b9526` is published in `SeniorAILab/SeeON-edge` release `models-onnx-2026-09-07`. The export owns a dynamic batch axis. Its SHA-256 is recorded as `onnx_sha256` in the engine identity, binding the built engine to this derived artifact; it does not add training-origin evidence beyond the source checkpoint. |
| `models/bed/yolo26l-seg.pt` | Normal `bed` runner (bed-exit) | **unknown** | **unknown** | SHA-256 `636024306410afa1732692322fba57d22ea2b1c2f07613fcee131a93d7dd380c` is known. `docs/adr/0006-model-handoff-contract.md` documents the bed artifact contract, but the sibling repository has no bed training script, dataset YAML, run log, or populated model provenance manifest. `manifests/models/README.md` is contract-only. |
| `models/bed/yolo26l-seg.onnx`, `models/bed/yolo26l-seg.onnx.sha256` | 1280-input bed-seg ONNX runner input and required digest sidecar; produced by `worker/tools/export_bed_seg_onnx.py --imgsz 1280` from the bed checkpoint | **unknown** | **unknown** | ONNX SHA-256 `de1c081df29936d6cf42329ec53b4d22f1e02f13bf1ea3b769907dc1d95a3d86` and its 65-byte sidecar are published in `SeniorAILab/SeeON-edge` release `models-onnx-2026-09-07`. This derived artifact adds no training-origin evidence beyond the source checkpoint. |
| `models/fall/pose-bbox56-gru/model.onnx` | CPU ONNX Runtime pose+bbox56 fall-bundle member; produced by `worker.tools.export_fall_onnx` from the pinned `model.pt` bundle member | **no** | **no** | SHA-256 `258ae9d9460534e659bf97af4bc55a083c830fa190ff6c8ed347db6bdbf32163`, published with the complete private bundle at `Berom0227/seeon-model-v0.1.0-pose-bbox56-proxy-research@2c46e52e52fe9319e5bcd5833bb0d0a66c062fb7`. The exporter verifies the source bundle and records this member in `bundle-manifest.json`. |
...
2. **Bed checkpoint:** For the known SHA-256 `636024306410afa1732692322fba57d22ea2b1c2f07613fcee131a93d7dd380c` of `yolo26l-seg.pt`, provide the same
