...
| `models/pose/yolo26n-pose.onnx` | Dynamic-batch engine-build input; produced by `worker/tools/export_pose_onnx.py` from the pose checkpoint | **unknown** | **unknown** | The export owns a dynamic batch axis. Its SHA-256 is recorded as `onnx_sha256` in the engine identity, binding the built engine to this derived artifact; it does not add training-origin evidence beyond the source checkpoint. |
