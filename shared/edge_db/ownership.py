...
WORKER_LEGACY_TABLES: Final = frozenset(
    {
        "clip_events",
        "config_current",
        "config_history",
...
TABLE_FAMILIES: Final = (
    ...
    TableFamily("runtime_", Writer.WORKER, "applied worker runtime state"),
    TableFamily("evidence_", Writer.WORKER, "event and evidence state"),
    TableFamily("derivative_", Writer.WORKER, "derived media state"),
...
def writer_for_table(table: str) -> Writer | None:
    ...
    if table in API_LEGACY_TABLES:
        return Writer.API
    if table in WORKER_LEGACY_TABLES:
