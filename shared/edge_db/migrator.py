...
from shared.edge_db.paths import EDGE_DATABASE_PATH, prepare_database_path, secure_database_files
from shared.edge_db.schema import MIGRATIONS, Migration
...
class _MigrationAuthorizer:
    ...
    def __call__(
        ...
        if action == sqlite3.SQLITE_CREATE_TABLE and argument_one is not None:
            ...
            self._creates_application_table = writer in {Writer.API, Writer.WORKER}
        if action == sqlite3.SQLITE_ALTER_TABLE and argument_two is not None:
            if writer_for_table(argument_two) in {Writer.API, Writer.WORKER}:
