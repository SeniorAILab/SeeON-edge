...
**Camera wall detection surface.** `CameraWall.tsx` subscribes to `useStatusResource(true)` exactly once (the shared 3s cadence, unchanged) and reads each camera's diagnostics by its own **local** camera id — the same key `RoomDetail` uses. Each `CameraWallTile` receives that camera's `RuntimeCameraDiagnostics` (required prop, `undefined` when the payload has none) and renders one `StatusBadge`-class detection pill under the media frame using the shared `detectionStatus` table: text plus shape signal, never color-only, on online and offline tiles alike, and never covering the snapshot/media region or the label bar. The tile badge itself carries no ARIA role. Above the grid, when **any currently visible** camera is `blind`, the wall renders exactly one persistent `role="alert"` summary (`data-testid="wall-detection-summary"`, `data-blind-count`) naming the affected cameras and the next operator action; it is page-resident, never a toast, and never auto-dismissed. A blind camera excluded by the floor filter produces no summary — the alarm describes what the operator is actually looking at. The first snapshot in which every visible camera leaves `blind` clears both the summary and the tile alarm state. Missing diagnostics render `unknown`, never a fabricated healthy.
...
- `CameraInfoCard.tsx` is now header-title-as-camera-name with an xs outline "연결 관리" button in the header (opens the shared `CameraEditModal` in place via `onManageConnection`) and a `dl` of 층/RTSP 주소(masked, `rtsp_url_masked`)/클라우드 연동/감지 상태/연결 이력 — the connection-상태 row and the old full-width bottom button are removed, since connection status now shows as the breadcrumb-row badge and 연결 관리 no longer navigates away.
- **감지 상태 row (operator detection alarm).** Connection, cloud mapping, and detection are three independent facts and each owns its own labelled row; a connected camera can simultaneously be detection-blind, and a blind alarm never hides or replaces the mapping row. The row renders `runtime.cameras[camera.id].detection.state` (`RoomDetail` passes it down; a missing/old-worker payload is `unknown`, never a fabricated healthy) as one `StatusBadge`-class pill carrying a shape signal plus Korean text, so meaning is never color-only: `healthy` → 감지 정상 (approved; a completed cycle with zero events IS healthy), `starting` → 감지 상태 확인 중 (closed), `unknown` → 감지 상태 확인 불가 (pending), `disabled` → 감지 비활성 (closed), `blind` → 감지 중단 (rejected). Only `blind` is a persistent high-priority alert: it is the sole state with `role="alert"`, it carries a `reason`-specific one-line Korean operator action (`pose_not_completing` / `decision_not_completing` / `no_completed_cycles` are distinct copy; `telemetry_*` and `counter_reset` have their own), and it is card-resident rather than a toast so it survives a shift change. Every other state uses `role="status"`. Recovery to `healthy` clears the alert on the next rendered snapshot — no latch, no auto-dismiss timer, no animation. `detectionStatus.tsx` owns the shared label/tone table, the reason copy, and the `DetectionStateIcon` shape signal (status dot vs. warning triangle SVG; no emoji) so the wall and the room-detail card never describe the same fact with different words. Guidance copy binds its noun phrases with `\u00a0` and renders with `break-keep` so Korean phrases never split across lines at 390px or 200% zoom. Accepted debt: `starting`/`disabled` reuse the existing `status-closed` pair (4.4:1), the same contrast the shipped `StatusBadge` 확인 중 state already carries — not a new token, and not reconciled here.
...
- `EventHistoryList.tsx` polls `GET /clips` scoped to the camera and offers a 최신순/종류별 sort select; 최신순 is a flat 4-col clip grid, 종류별 groups clips under `"{종류} · n건"` headings. A tile never mounts video: `clip.thumbnail_available` gates the shared `ClipThumbnail`'s authenticated, same-origin thumbnail `<img loading="lazy">` (explicit 640×360 dimensions, contained inside the stable 16:9 frame) versus the existing playable/unavailable placeholder. An image load error switches to that same placeholder without disabling selection. The type label and timestamp remain below; selecting any `video_available` clip, including one without a thumbnail, opens `ClipPlayerModal` (`front/src/features/operations/ClipPlayerModal.tsx`, `size="xl"`, same shape as the 이벤트 page's `ClipPlaybackModal`).

## 이벤트 (Events)

`front/src/app/pages/EventsPage.tsx`: a filter row of `EventTypeFilterChips.tsx` (전체 n / 침대 이탈 n / 낙상 n) on the left and `CameraFilterSelect.tsx` on the right (`ml-auto`), driving a 4-col `ClipGrid.tsx` (each card labels its camera in the top-left of the media frame via `resolveCameraLabel.ts`). Counts come from the backend's exact `event_type_counts` facet, not from the visible page. Both filters are sent to `GET /clips` as `camera_id` and `event_type`; changing either filter resets the local page to 1. Filter chips and the camera select use a 44px minimum hit target at every viewport. The empty state reads `조건에 맞는 이벤트가 없습니다.`. Selecting a clip opens `ClipPlaybackModal.tsx`. TP/FP labeling (the old `PUT /clips/{id}/label` review workflow) is intentionally removed from this redesign — the modal shows the event-type chip in its place, and the label-mutation call is deleted from the frontend entirely (backend route removal is out of scope).

**Events pagination policy:** the page size is an explicit fixed product policy of **48 clips** (`EVENTS_PAGE_SIZE`), producing at most 48 clip-card buttons in the DOM. `EventsPager.tsx` is the reusable numbered pager primitive: previous/next controls, a compact first/current-neighbor/last page window with ellipses, `aria-current="page"`, a visible `현재 / 전체 페이지` summary, and a result-range summary. Page state is local to `EventsPage`; it never adds a URL query key. Pager requests are transactional: the visible page number and cards change only after the requested page succeeds, so a failed navigation retains the last-good current page. Retention correction is bounded to the requested page, the response's reported last page, then page zero if that correction is already invalid; only a valid bounded response commits. Page 1 polls every 8 seconds; historical pages have an explicit `현재 페이지 새로 고침` control and never auto-poll. Last-good data renders a visible `마지막 확인` timestamp, background refresh is labelled without blocking review, and a failed refresh keeps the cards with an immediate `다시 시도` action.

Clip cards remain metadata-only controls and never mount `<video>` or `<source>`. When `thumbnail_available` is true, `ClipThumbnail` derives `/api/v1/clips/{encoded clip id}/thumbnail` from the normalized clip id rather than trusting any server path, then renders a meaningful Korean-alt `<img loading="lazy" width="640" height="360">` with `object-contain` inside the stable 16:9 media frame. A missing thumbnail or image load error keeps the existing play placeholder for a playable clip; unavailable video keeps the unavailable-copy placeholder. Either fallback preserves the card button and therefore preserves playback selection. A failed thumbnail request applies only to that exact normalized clip object: a fresh clip DTO from polling, a changed clip id/derived URL, or availability changing back to true retries the lazy image without mounting video or preloading all cards. Only the selected `ClipPlaybackModal` mounts exactly one native `<video controls autoPlay playsInline preload="metadata">`; `AutoplayVideo` also calls `play()` when mounted so a ready clip starts without a second click. Its exact autoplay and media-failure states are defined under **Clip media states** below. A legacy response without `pagination` is treated as the complete response, filtered and sliced client-side to the same 48-item policy, with exact local facets derived before the event-type filter. An off-page `clip` deep link is resolved from that complete legacy envelope when available; otherwise it is preserved while `GET /clips/{clip_id}/metadata` resolves metadata. Successful metadata opens the modal only when it matches the active event and camera filters. A 400 or 404 response removes the invalid key with `replaceState`; a transient/network/5xx failure preserves the key and renders a retryable modal state.

## 설정 (Settings)

`front/src/app/pages/SettingsPage.tsx`: a 7:5 two-column grid (`lg:grid-cols-12`, 7/5 span) — `CameraSection` on the left; `DetectionSettingsCard`, `ConnectionSettingsPanel`, `TopologyEditor`, `ProcessingStatusCard`, `ClipStorageCard` stacked on the right.
...
- **장치 등록** (`ConnectionSettingsPanel.tsx`, `front/src/features/connection/`): read mode shows the deployment-owned backend origin, facility code, canonical facility ID, installation generation, masked token, and last synchronization. Edit mode accepts exactly facility code and password token. One in-memory `clientInstallationRef` remains stable through test/save attempts and becomes server-persisted after a successful save; it is never rendered or placed in browser storage. Save/test errors are localized by status without rendering backend bodies.
- **시설 토폴로지** (`TopologyEditor.tsx`, `front/src/features/cameras/`): a stack layout owns local topology status, current/dirty registry versions, floor/room create and rename forms, and explicit existing-camera pairing using stable camera and room refs. Sync is an explicit button, never polling or auto-confirming. A durable omission preview displays only redacted counts and client/server revisions. `TopologyConfirmationDialog` re-fetches and compares the exact preview identity immediately before destructive confirmation, blocks expired/changed previews and double submits, and calls only the local authenticated confirmation route.
...
`CameraRegisterModal` is a single compact registration form: there is no floor-chip picker, registration-local confirmation step, or live bed-zone view. `floor_name` is a read-only field populated by an external space-sync process; the form submits 이름, RTSP 주소, floor override, and selected space. `POST /cameras` performs the RTSP probe and save in one call (no separate probe endpoint). On success it shows the existing success toast, refreshes the registry once, and closes immediately. A probe failure (`timeout`/`auth`/`decode`) or duplicate-stream conflict (with a "그래도 등록" force-register retry) remains inline. `BedZoneRecognitionPanel` remains exclusive to `CameraEditModal`'s 다시 인식 flow, where it uses MJPEG plus an SVG polygon overlay and saves through `POST /cameras/{camera_id}/bed-zone/recognize`.

## 모달과 AccessibleDialog 크기 (Modals)

All settings and evidence modals use `AccessibleDialog` (`front/src/shared/ui/AccessibleDialog.tsx`) with a `DialogSize` variant matching the handoff spec's pixel widths:
...
| `md` | 440px | 카메라 등록 · 연결 관리(view mode) | `CameraRegisterModal.tsx` · `CameraEditModal.tsx` (mode `view`) |
| `lg` | 640px | 연결 관리 다시 인식 | `CameraEditModal.tsx` (mode `reseg`) |
...
**클립 재생** (`ClipPlaybackModal.tsx`/`ClipPlayerModal.tsx`, xl): header is `"{종류} · {카메라명}"` + timestamp; body is exactly one native `<video controls autoPlay playsInline preload="metadata">` (or an unavailable-copy fallback) inside `.event-media-frame`, then a `dl` of 카메라/시간/길이/해상도/크기(only when `size_bytes` is non-null — the row is omitted, never shown as `0` or fabricated). `AutoplayVideo` explicitly calls `play()` on mount in addition to declaring `autoPlay`; native controls remain operable in every state, and status/retry content stays at the top of the media frame without covering the controls. 길이 prefers the clip manifest's `duration_s`, falling back to the loaded `<video>` element's metadata; 해상도 is read only from loaded `<video>` metadata (the API's `Clip` type carries no dimensions field). Deviation from the handoff spec: there is no "파일" row showing the full clip-store path — `AGENTS.md` prohibits exposing the clip-store filesystem path or camera credentials in the UI, so only `video_path` (an API URL, never a raw filesystem path) is used, and the path row is dropped entirely rather than shown redacted. Footer has one action, 다운로드 (`<a download>` on `video_path`), shown only when `video_available`.
...
- **Bed-zone recognition** — `POST /api/v1/cameras/{camera_id}/bed-zone/recognize` (dashboard session auth) triggers one server-side YOLO segmentation pass against the camera's latest frame; success (200) persists `{polygon, image_width, image_height, recognized_at}` server-side (`BedZoneStore`) and is echoed back under `bed_zone`. No usable bed found → 422 `{"detail": {"error_class": "bed_not_found"}}`, surfaced as "침대를 찾지 못했습니다…"; any other upstream failure (worker down, no frame, decode failure) → 503, surfaced as a generic retry message. `GET /cameras` includes each camera's persisted `bed_zone: {...}|null`, which drives the 인식 완료/인식 필요 badge wherever it appears (카메라 테이블 and 연결 관리).
...
| Unavailable media | Keep identity and metadata visible inside a stable dark frame; a missing/failed thumbnail uses the playable placeholder when video remains available, while unavailable video shows `영상을 불러올 수 없음`. |
...
`AutoplayVideo` and `ClipThumbnail` use the following exact state contract; autoplay policy and media transport/decode failure must never share operator copy.

| Primitive state | Trigger | Presentation | Recovery |
| --- | --- | --- | --- |
| Video ready | Initial mount/source change, fulfilled `play()`, native `play`, or `loadeddata` | Exactly one native video with `controls`, `autoPlay`, `playsInline`, and `preload="metadata"`; no notice. | Current playback continues. A source change clears every prior failure and attempts autoplay for the new source. |
| Autoplay blocked | `play()` rejects with a real `DOMException` whose `name` is exactly `NotAllowedError`. | Polite `role="status"`: `자동 재생이 차단되었습니다. 재생 버튼을 눌러 영상을 확인하세요.` Native controls remain available. | Native play succeeds and clears the notice. No autoplay workaround, mute hack, or playback-rate change is allowed. |
| Video media failure | Any other `play()` rejection, including `NotSupportedError`, or the video element emits `error` for a network, transport, or decode failure. | `role="alert"`: `영상을 재생하지 못했습니다.` plus `다시 시도`; native controls remain available. This state must not mention autoplay policy. | `다시 시도` clears the stale notice, calls native `load()`, then calls `play()` again. A fulfilled retry, native `play`, `loadeddata`, or source change returns to ready; another rejection/error returns to the truthful failure state. |
| Thumbnail image | `thumbnail_available=true` and the current clip object's image request has not failed. | One meaningful-alt 640×360 `<img loading="lazy">` with `object-contain`; never video or `<source>`. | Successful `load` clears any prior failed identity. |
| Thumbnail playable fallback | `thumbnail_available=false`, or the current clip object's thumbnail emits `error` while `video_available=true`. | Existing play placeholder; the enclosing card remains selectable. | A fresh normalized clip object, changed clip id/derived thumbnail URL, or refreshed availability retries the lazy image once through normal rendering. No eager preload or card video is introduced. |
| Thumbnail unavailable fallback | `video_available=false`. | Stable dark frame with concise unavailable copy; no image or video request. | Fresh clip truth replaces the state through normal rendering. |
...
| `clip` | Exact clip `id` | `events` | Clip event and camera must match active filters. Paged lists validate off-page ids through `GET /clips/{clip_id}/metadata` before removing a key; an incomplete visible page is never treated as complete history. |
