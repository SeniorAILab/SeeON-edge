...
- `features/<slice>/`: components + view logic for one capability. A slice may add its own `AGENTS.md`.
  - `operations/`: camera wall, room detail, live MJPEG, overlay, snapshot queue, room event history.
...
- `shared/`: cross-slice building blocks only.
  - `shared/api/`: HTTP (`http.ts`), session/base URL (`session.ts`), `client.ts` + per-resource normalizers, DTO types, polling hooks (`usePollingResource.ts`), topology client.
...
- `settings` → `operations` (`BedZoneRecognitionPanel` reuses `useMjpegStream`).
