...
export function ClipPlaybackModal({
  ...
  return <AccessibleDialog open={open} title={title} onClose={onClose} size="xl" initialFocus="heading">
    ...
    <div className="event-media-frame relative mt-4">{clip.video_available ? analysis.settled ? <><AutoplayVideo key={videoSrc} src={videoSrc} className="h-full w-full object-contain" onVideoElement={setVideo} onLoadedMetadata={(nextVideo) => { setMetadata({ duration: nextVideo.duration, width: nextVideo.videoWidth, height: nextVideo.videoHeight }); }} />{video && timingUsable && analysis.status.result ? <ClipOverlayCanvas video={video} result={analysis.status.result} personEnabled={targets.person} bedEnabled={targets.bed} /> : null}</> : null : <div className="event-media-unavailable h-full px-4 text-center text-sm" data-testid="clip-modal-unavailable">{clip.video_error ?? '저장된 영상을 사용할 수 없습니다.'}</div>}</div>
