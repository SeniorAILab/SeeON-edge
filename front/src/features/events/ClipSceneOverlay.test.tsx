...
afterEach(() => { roots.forEach((root) => root.unmount()); roots.clear(); document.body.replaceChildren(); });
...
const frame = { t: 0, p: 0, q: 0, sd: false, dc: [], bd: [{ b: [1, 2, 3, 4], c: 1, i: 0, pg: [[1, 2], [3, 4]], pv: '', sm: '', ct: [] }], ps: [{ b: [10, 20, 30, 40], c: 1, i: 0, tr: null, k: [[0, null, null, .8], [1, 30, 40, .5]] }], lb: [{ x: 4, y: 5, t: 'late', c: [9, 8, 7], z: 9 }, { x: 1, y: 2, t: 'early', c: [6, 5, 4], z: 1 }] } as ClipSceneFrame;
...
describe('ClipSceneOverlay', () => {
  it('uses received colors and z order, skips null-coordinate keypoints, and preserves source viewBox', () => {
    ...
    expect(host.querySelector('rect')?.getAttribute('stroke')).toBe('rgb(10 11 12)');
