...
type InternalEntry = SnapshotEntry & { active: boolean; requestIssued: boolean; requestKey: number; requestTimer: number | null };

/**
 * Concurrency-capped one-shot loader for camera-wall thumbnails. Each identity requests at most
 * once per enabled activation, and each in-flight request settles into `error` after 10 seconds.
 * `useSyncExternalStore`-friendly via subscribe/snapshot.
 */
export class SnapshotQueue {
  ...
  constructor(private readonly urlFor: (mediaId: string, requestKey: number) => string, private readonly limit = 6) {}
  ...
  update(identities: readonly SnapshotIdentity[], enabled: boolean): void {
    ...
    for (const { id, mediaId } of identities) {
      ...
      this.entries.set(id, { id, mediaId, state: 'loading', requestUrl: null, lastLoadedUrl: null, lastLoadedAt: null, active: false, requestIssued: false, requestKey: 0, requestTimer: null });
    }
    if (!enabled) {
  ...
  private drain(): void {
    ...
    for (const id of this.order) {
      const entry = this.entries.get(id);
      if (!entry || entry.active || entry.requestIssued || entry.requestUrl !== null) continue;
      entry.active = true;
      entry.requestIssued = true;
      entry.requestUrl = this.urlFor(entry.mediaId, entry.requestKey);
      entry.requestKey += 1;
